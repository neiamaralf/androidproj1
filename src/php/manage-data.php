<?php
   ...
   switch($key){
     ...
     $imgfnome=$_REQUEST['fnome'];
      $myfile = fopen('./imagens/'.$imgfnome, "wb") or die("Erro ao abrir arquivo!");
      ...
      fclose($myfile); 
         ...
         try {           
          if($tabela=="produtos"||$tabela=="certificadoras")
            $sql 	= "UPDATE $tabela SET imagem='$imgfnome' WHERE id = $id";
           else if($tabela=="userimages")
            $sql 	= "INSERT INTO  $tabela(idusuario,imagem) VALUES($idusuario,'$imgfnome')";
           $qr = $pdo->query($sql);
           if($qr&&$qr->rowCount()>0){               
    ...
    break;
     ...
     case "insertum":         
      ...
      foreach($prodlist as $prod){
       $values=$values.'('.$idusuario.','.$prod['idproduto'].','.floatval(str_replace(',','.',$prod['preco'])).'),';
      }
      $values=rtrim($values,',');      
      $sql = "INSERT INTO listaprodutos(idusuario,idproduto,preco) VALUES $values";
       $qr = $pdo->query($sql);
       if($qr){               
             echo json_encode(array('insert' => "ok"));     
           }    
